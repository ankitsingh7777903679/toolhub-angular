...
@Component({
  selector: 'app-webp-to-jpg',
  standalone: true,
  imports: [CommonModule, FormsModule, SendToToolComponent],
  templateUrl: './webp-to-jpg.component.html',
  styleUrl: './webp-to-jpg.component.scss'
})
export class WebpToJpgComponent implements OnInit {
  ...
  get firstFileName(): string {
    return this.files.find(f => f.status === 'done')?.name || 'converted.jpg';
  }
  ...
  async convertAll(): Promise<void> {
    ...
    for (const fileItem of this.files) {
      ...
      try {
        const result = await this.convertWebPToJpg(fileItem.previewUrl);
  ...
  private convertWebPToJpg(src: string): Promise<{ dataUrl: string, blob: Blob }> {
  ...
  downloadZip(): void {
    // Download all files individually since we don't have jszip
